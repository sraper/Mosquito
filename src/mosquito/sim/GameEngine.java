...
public final class GameEngine 
{
	...
	public boolean step()
	{
		if(board.mosquitosCaught >= (config.getNumMosquitos() * config.threshold / 100) || (config.getMaxRounds() > 0 && getCurrentRound() > config.getMaxRounds()))
		{
			//GAME OVER!
	...
	public boolean setUpGame()
	{
		try
		{
			...
			if(this.isSimulated)
				...
				board.playerLines = curPlayer.startNewGame(board.getWalls(), config.getNumLights());
			...
			*/
			...
			collectors.add(curPlayer.getCollector()); 
			if(collectors == null)
			{
