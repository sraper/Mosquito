...
public final class Board {
    ...
    protected ArrayList<ArrayList<Point2D>> allPoints;
        ...
        try {
            for (Light l : lights) {
                ...
                if (l.isOn(engine.getCurrentRound())
                    ...
                    Line2D between = new Line2D.Double(p.getX(), p.getY(), l
                            .getLocation().getX(), l.getLocation().getY());
                    for (Line2D w : walls) {
    ...
    public double getDirectionOfLight(Point2D p) {
        ...
        double max_dist = 21;
        for (Light l : lights) {
            if (l.getLocation().distance(p) < max_dist
                    && l.isOn(engine.getCurrentRound())) {
                ...
                if (!obscured) {
                    ...
                    angle = Math.atan(Math.abs((l.getLocation().getY() - p
                            .getY()) / (l.getLocation().getX() - p.getX())))
                            * 180 / Math.PI;

                    if (l.getLocation().getX() > p.getX()
                            && l.getLocation().getY() < p.getY()) {
                        angle = angle;
                    } else if (l.getLocation().getX() < p.getX()
                            && l.getLocation().getY() > p.getY()) {
                        angle = 180 + angle;
                    } else if (l.getLocation().getX() < p.getX()
                            && l.getLocation().getY() < p.getY()) {
