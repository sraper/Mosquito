		...
		if (engine != null && board.getLights() != null) {
			...
			for (Light l : board.getLights()) {
				...
				for (int i = 0; i < myPoints.size()-1;i++) {
					int oldX = (int) Board.toScreenSpace(myPoints.get(i).getX());
					int oldY = (int) Board.toScreenSpace(myPoints.get(i).getY());
					int newX = (int) Board.toScreenSpace(myPoints.get(i+1).getX());
					int newY = (int) Board.toScreenSpace(myPoints.get(i+1).getY());
